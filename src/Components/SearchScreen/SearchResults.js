                                ...
                                <div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                                    <table className="min-w-full divide-y divide-slate-800">
                                        ...
                                        <tr>
                                            ...
                                            <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-white">State</th>
                                        </tr>
                                        </thead>
                                            ...
                                            data.result.map(customer => (
                                                <tr className="hover:bg-slate-500/10">
                                                    <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-white sm:pl-0">
                                                        <a href={`/app/common/entity/custjob.nl?id=${customer.id}`} target="_blank" rel="noreferrer" className="text-indigo-400 hover:text-indigo-300">
                                                            {customer.companyName}
                                                        </a>
                                                    </td>
                                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-300">{customer.email}</td>
                                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-300">{customer.phone}</td>
                                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-300">{customer.billingCity}</td>
                                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-300">{customer.billingState}</td>
                                                </tr>))}
                                        {(!data ||
                                            !data.result?.length) &&
                                            (
