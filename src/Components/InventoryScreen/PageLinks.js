...
function PageLinks({ currentPage, maxPage }) {
    ...
    const baseClasses = 'inline-flex items-center px-4 pt-4 text-sm font-medium';
    ...
    return (
        <nav className="flex items-center justify-between border-t border-gray-700 px-4 mt-2 mb-8 sm:px-0">
            <div className="-mt-px flex w-0 flex-1">
                {currentPage !== 1 && (
                    <Link to={urlForPage(currentPage - 1)}
            ...
            <div className="hidden md:-mt-px md:flex">
                ...
                {pageNumbers.map(page => (
                    <Link key={page} to={urlForPage(page)} className={page === currentPage ? activeClasses : defaultClasses}>{page}</Link>
                ))}
